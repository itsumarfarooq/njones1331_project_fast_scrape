...
class EventectiveScraperSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for a_elem in response.xpath('//div[@class="innlocationSec"]//ul/li/a'):
            ...
            state_name = a_elem.xpath('./text()').get().strip() if a_elem.xpath(
                './text()').get().strip() else a_elem.xpath('./strong/text()').get().strip()
            yield scrapy.Request(url+"?alpha=AZ", callback=self.scrape_cities, cb_kwargs=dict(state_name=state_name), dont_filter=True)
    ...
    def scrape_cities(self, response, state_name):
        for a_elem in response.xpath('//div[@class="innlocationlink row"]//ul/li/a'):
            ...
            yield scrapy.Request(url, callback=self.scrape_events, cb_kwargs=dict(state_name=state_name, city_name=city_name), dont_filter=True)

    def scrape_events(self, response, state_name, city_name):
        for url in response.xpath('//a[@class="provider-name"]/@href').getall():
            yield dict(state_name=state_name, city_name=city_name, url=url)
        next_page_url = response.xpath('//a[@aria-label="Next"]/@href').get()
        if next_page_url:
            yield response.follow(next_page_url, callback=self.scrape_events, cb_kwargs=dict(state_name=state_name, city_name=city_name), dont_filter=True)

    # def scrape_event_data(self, response, state_name, city_name):
    #     event_info = {}
    #     event_info['URL'] = response.url
    #     event_info['Title'] = response.xpath('//h1/text()').get()
    #     event_info['State'] = state_name
    #     event_info['City'] = city_name
